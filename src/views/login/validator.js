...
export default {
  ...
  passwordPattern (rule, value) {
    if (value === '') {
      return Promise.reject(new Error('密码不能为空！'))
    } else if (!passwordReg.test(value)) {
      return Promise.reject(new Error('密码最少6位，包括至少1个大写字母，1个小写字母，1个数字'))
    } else {
      return Promise.resolve()
    }
  },
  passwordQesPattern (rule, value) {
    if (value === '') {
      return Promise.reject(new Error('密码保护问题不能为空！'))
    } else {
      return Promise.resolve()
    }
  },
  passwordAnsPattern (rule, value) {
    if (value === '') {
      return Promise.reject(new Error('密码保护问题答案不能为空！'))
    } else {
      return Promise.resolve()
    }
  }
